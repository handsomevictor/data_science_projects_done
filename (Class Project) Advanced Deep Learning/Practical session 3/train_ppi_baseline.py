...
class GAT(nn.Module):
    ...
    def __init__(self, g, input_size, output_size, nonlinearity):
        ...
        self.g = g
        self.layers = nn.ModuleList()
        self.layers.append(GATConv(input_size, 256, 4, activation=nonlinearity))
        self.layers.append(GATConv(256 * 4, 256, 4, activation=nonlinearity))
        self.layers.append(GATConv(256 * 4, output_size, 6))
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i < len(self.layers)-1:
                outputs = outputs.view(-1, outputs.size(1) * outputs.size(2))
        outputs = outputs.mean(1)  # Average over the 6 heads
        outputs = nn.Sigmoid()(outputs)  # Apply a sigmoid
...
def main(args):
    ...
    model = AttentionGraphModel(g=train_dataset.graph, layer_sizes=layer_sizes, input_size=n_features,
                                ...
                                num_heads=10).to(device)
    """

    # n_layers=2, hidden_size=256, num_heads=4  -> train loss: 0.3687; test f1: 0.6609
    # n_layers=2, hidden_size=256, num_heads=10 -> train loss: 0.2789; test f1: 0.7720
