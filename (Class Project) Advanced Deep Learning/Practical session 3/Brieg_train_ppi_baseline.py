...
class ConvGraphModel(nn.Module):z
...
def main(args):
    ...
    # n_layers=2, hidden_size=256 -> train loss: 0.4497; train f1: 0.5105; test f1: 0.5081

    """
    layer_sizes = [256, 256]
    model = AttentionGraphModel(g=train_dataset.graph, layer_sizes=layer_sizes, input_size=n_features,
                                output_size=n_classes, nonlinearity=F.elu,
                                num_heads=4).to(device)
    """

    # n_layers=2, hidden_size=256, num_heads=4 -> train loss: 0.2847; train f1: 0.7432; test f1: 0.7367

    """
    model = GAT(g=train_dataset.graph, input_size=n_features,
                output_size=n_classes, nonlinearity=F.elu,
                n_layers=2, num_heads=4, skip_connections=False).to(device)
    """

    # no skip connection, n_layers=2, 250 epochs -> train loss: 0.1634; train f1: 0.8633; test f1: 0.8450
    # skip connection, n_layers=2, 250 epochs -> train loss: 0.1407; train f1: 0.8595; test f1: 0.8559
    # skip connection, n_layers=3, 250 epochs -> train loss: 0.0597; train f1: 0.9375; test f1: 0.9298
